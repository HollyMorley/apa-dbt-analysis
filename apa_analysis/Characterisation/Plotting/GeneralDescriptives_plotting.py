"""General descriptive statistic plots for feature distributions."""
import pandas as pd
...
def plot_toe_distance_to_transition(raw_data, mouse_runs, phases, savedir, fs=7, n_interp=100, stsw='st', bodypart='Toe', raw=False):
    ...
    if raw:
        ...
        fig_scatter, ax_scatter = plt.subplots(figsize=(4, 2))

    all_arr = {}
    stride_lengths_all = {}
    dist_to_transition_all = {}
    for phase in phases:
        ...
        for m, mouse in enumerate(mice):
            ...
            for r, run in enumerate(phase_runs):
                ...
                stance_periods_idxs = run_data.loc(axis=0)['RunStart'].index.get_level_values(level='FrameIdx')[
                    stance_periods_mask]
                stance_chunks = utils.Utils().find_blocks(stance_periods_idxs, gap_threshold=3, block_min_size=3)
                last_stance_idx = stance_chunks[-1][0]

                # stance_periods_mask = \
                # run_data.loc(axis=0)['RunStart'].loc(axis=1)[
                #     transition_paw, 'SwSt_discrete'] == \
                # locostuff['swst_vals_2025']['st']
                # stance_periods_idxs = \
                # run_data.loc(axis=0)['RunStart'].index.get_level_values(
                #     level='FrameIdx')[
                #     stance_periods_mask]
                # last_stance_idx = stance_periods_idxs[-1]
                ...
                if stsw == 'st':
                    ...
                    if len(sw_start_idxs) == 0:
                        print(f"skipped run {run} for mouse {mouse} because no swing start found")
                        continue
        ...
        stride_lengths_mice = np.full(num_mice, np.nan)
        dist_to_transition_mice = np.full(num_mice, np.nan)
        for mouse in range(num_mice):
            first_xs = arr[mouse, :, 0, 0]
            last_xs = arr[mouse, :, 0, 1]
            mouse_stride_lengths = last_xs - first_xs
            stride_lengths_mice[mouse] = np.nanmean(mouse_stride_lengths)
            dist_to_transition_mice[mouse] = np.nanmean(expstuff['setup']['transition_mm'] - first_xs)
        stride_lengths_all[phase] = stride_lengths_mice
        dist_to_transition_all[phase] = dist_to_transition_mice
        ...
        if not raw:
            ...
            ax_scatter.scatter(first_mean_x, first_mean_y, color=color, label='Start', marker='x', s=10, zorder=5)
            ax_scatter.scatter(last_mean_x, last_mean_y, color=color, label='End', marker='x', s=10, zorder=5)
            ax_scatter.plot([first_mean_x, last_mean_x], [first_mean_y, last_mean_y], color=color, linewidth=0.8, zorder=4)
            ax_scatter.plot([470,470], [0,50], color='k', linestyle='--', linewidth=0.5, zorder=0)
        else:
            color = pu.get_color_phase(phase)
            ...
            for mouse in range(num_mice):
                ...
                ci_multiplier = 2.262  # t critical value for 95% CI, df=8 (n_phase_runs-1), adjust if needed

                for xs, pos in zip([first_xs_valid, last_xs_valid], positions):
                    n = len(xs)
                    if n < 2:
                        continue
                    mean = np.nanmean(xs)
                    ci = stats.t.ppf(0.975, df=n - 1) * (
                                np.nanstd(xs, ddof=1) / np.sqrt(n))
                    axs[mouse].errorbar(mean, pos, xerr=ci,
                                        fmt='o', color=color,
    ...
    if not raw:
        ...
        fig_stride, ax_stride = plt.subplots(figsize=(4, 2.5))
        for mouse in range(stride_lengths_all_arr.shape[0]):
            ax_stride.plot([0, 1], stride_lengths_all_arr[mouse, :], 'o-', alpha=0.3, color='grey', markersize=3, zorder=10)
        ...
        print(f"Wilcoxon stride lengths {phases[0]} vs {phases[1]}: t={t:.3f}, p={p:.4f}")

        ax_stride.set_xticks([0, 1])
        ax_stride.set_xticklabels([phases[0], phases[1]], fontsize=fs)
        ax_stride.set_ylabel('Stride length (mm)', fontsize=fs)
        ax_stride.tick_params(axis='y', labelsize=fs)
        savepath_stride = os.path.join(savedir, f"stride_lengths_{phases[0]}_{phases[1]}_{stsw}_{bodypart}")
        fig_stride.savefig(f"{savepath_stride}.png", dpi=400)
        fig_stride.savefig(f"{savepath_stride}.svg", dpi=400)

        # Distance to transition paired plot (directly comparable to PC7 feature)
        dist_to_transition_arr = np.array([dist_to_transition_all[phases[0]], dist_to_transition_all[phases[1]]]).T  # shape (num_mice, 2)
        fig_dist, ax_dist = plt.subplots(figsize=(4, 2.5))
        for mouse in range(dist_to_transition_arr.shape[0]):
            ax_dist.plot([0, 1], dist_to_transition_arr[mouse, :], 'o-', alpha=0.3, color='grey', markersize=3, zorder=10)
        t_dist, p_dist = stats.wilcoxon(dist_to_transition_arr[:, 0], dist_to_transition_arr[:, 1])
        print(f"Wilcoxon dist-to-transition {phases[0]} vs {phases[1]}: t={t_dist:.3f}, p={p_dist:.4f}")
        ax_dist.set_xticks([0, 1])
        ax_dist.set_xticklabels([phases[0], phases[1]], fontsize=fs)
        ax_dist.set_ylabel(f'Distance to transition ({stsw}, mm)', fontsize=fs)
        ax_dist.tick_params(axis='y', labelsize=fs)
        savepath_dist = os.path.join(savedir, f"dist_to_transition_{phases[0]}_{phases[1]}_{stsw}_{bodypart}")
        fig_dist.savefig(f"{savepath_dist}.png", dpi=400)
        fig_dist.savefig(f"{savepath_dist}.svg", dpi=400)
    ...
    def set_formatting(ax, violin):
        ...
        ax.set_xlim(340, 600)
        ax.set_xticks([340, 470, 600])
        ax.set_xticklabels([340, 470, 600], fontsize=fs)
    ...
    if raw:
        ...
        set_formatting(ax_scatter, violin=False)
        savepath = os.path.join(savedir, f"toe_distance_to_transition_{phases[0]}_{phases[1]}_{stsw}_{bodypart}")
        fig_scatter.savefig(f"{savepath}.png", dpi=400)
        fig_scatter.savefig(f"{savepath}.svg", dpi=400)


def plot_toe_distance_to_transition_paired(raw_data, mouse_runs, phases, savedir, fs=7, stsw='st', bodypart='Toe', raw=False):
    mice = ['1035243', '1035244', '1035245', '1035246', '1035250', '1035297', '1035299', '1035301']
    y_midline = structural_stuff['belt_width'] / 2
    ...
    for phase in phases:
        ...
        for m, mouse in enumerate(mice):
            ...
            for r, run in enumerate(phase_runs):
                ...
                paw_transition_position = \
                    run_data.loc(axis=1)[paw, 'x'].loc(axis=0)['Transition'].iloc[0]
                # if paw_transition_position < expstuff['setup']['transition_mm']:
                #     continue
                #
                # # check for sliding trials and skip them
        ...
        if not raw:
            ...
            # ── Y-band allocation ──────────────────────────────────────────────
            # Each phase occupies a 50-unit band; phase index 0 → top, 1 → bottom
            # (matching the original interval logic: phase 0 centred at 50-interval,
            #  phase 1 centred at interval, so phase 0 is higher on the axis)
            phase_idx = phases.index(phase)
            band_height = 50          # full height of one phase's band per mouse subplot
            band_centre = (band_height/2) + (1 - phase_idx) * 50  # phase 0 → 75, phase 1 → 25
            # We stack individual trials as evenly-spaced horizontal lines within
            # a ±line_spread window around the band centre
            line_spread = 15          # half-height of the trial-line region

            for mouse_idx in range(num_mice):
                first_xs = arr[mouse_idx, :, 0, 0]   # shape (n_phase_runs,)
                last_xs  = arr[mouse_idx, :, 0, 1]

                # Only include trials where both endpoints are valid
                valid_mask = ~np.isnan(first_xs) & ~np.isnan(last_xs)
                first_xs_valid = first_xs[valid_mask]
                last_xs_valid  = last_xs[valid_mask]
                n_valid = len(first_xs_valid)

                if n_valid == 0:
                    continue

                # Assign each trial a unique y-position within the band
                if n_valid == 1:
                    y_positions = np.array([band_centre])
                else:
                    y_positions = np.linspace(
                        band_centre - line_spread,
                        band_centre + line_spread,
                        n_valid
                    )

                # Draw individual paired lines
                for fx, lx, yp in zip(first_xs_valid, last_xs_valid, y_positions):
                    axs[mouse_idx].plot(
                        [fx, lx], [yp, yp],
                        color=color, alpha=0.25, linewidth=0.6, zorder=3
                    )
                    # Mark endpoints with small dots
                    axs[mouse_idx].plot(fx, yp, 'o', color=color, markersize=1.5,
                                        alpha=0.4, zorder=4)
                    axs[mouse_idx].plot(lx, yp, 'o', color=color, markersize=1.5,
                                        alpha=0.4, zorder=4)

                # ── Mean points ──────────────────────────────────────────────
                mean_first_x = np.nanmean(first_xs_valid)
                mean_last_x  = np.nanmean(last_xs_valid)

                # axs[mouse_idx].plot(
                #     [mean_first_x, mean_last_x],
                #     [band_centre, band_centre],
                #     color=color, linewidth=2.0, zorder=6, solid_capstyle='round'
                # )
                axs[mouse_idx].plot(mean_first_x, band_centre, 'o',
                                    color=color, markersize=4, zorder=7,
                                    markeredgecolor='k', markeredgewidth=0.4)
                axs[mouse_idx].plot(mean_last_x, band_centre, 'o',
                                    color=color, markersize=4, zorder=7,
                                    markeredgecolor='k', markeredgewidth=0.4)

                # Transition line
                axs[mouse_idx].plot([470, 470], [0, 100], color='k',
                                    linestyle='--', linewidth=0.5, zorder=0)
                axs[mouse_idx].set_title(f"{mice[mouse_idx]}", fontsize=fs)

    if not raw:
        stride_lengths_all_arr = np.array([stride_lengths_all[phases[0]], stride_lengths_all[phases[1]]]).T
    ...
    def set_formatting(ax, violin):
        ...
        if raw:
            ax.set_ylim(0, 100)
            ax.set_yticks([25, 75])
            ax.set_yticklabels([phases[1], phases[0]], fontsize=fs)  # lower band = phase 1, upper = phase 0
        else:
            ax.set_ylim(0, 50)
            ax.set_yticks(np.arange(0, 51, 25))
            ax.set_yticklabels(np.arange(0, 51, 25), fontsize=fs)
    ...
    if raw:
        for mouse_idx in range(num_mice):
            set_formatting(axs[mouse_idx], violin=True)
        savepath = os.path.join(savedir, f"toe_distance_to_transition_paired_{phases[0]}_{phases[1]}_{stsw}_{bodypart}_all_mice")
    else:
        set_formatting(ax, violin=False)
        savepath = os.path.join(savedir, f"toe_distance_to_transition_paired_{phases[0]}_{phases[1]}_{stsw}_{bodypart}")
...
def handedness(raw_data):
    allowed_mice = condition_specific_settings['APAChar_LowHigh']['global_fs_mouse_ids']
    mice = [m for m in raw_data.keys() if m in allowed_mice]
...
step_save_dir = r"H:\Characterisation_v2\Step_positions"
if not os.path.exists(step_save_dir):
    os.makedirs(step_save_dir)

# plot_toe_distance_to_transition_paired(raw_data, mouse_to_runs, phases=['APA2', 'Wash2'], savedir=step_save_dir, fs=7, stsw='sw', bodypart='Toe', raw=True)
# plot_toe_distance_to_transition_paired(raw_data, mouse_to_runs, phases=['APA2', 'Wash2'], savedir=step_save_dir, fs=7, stsw='st', bodypart='Toe', raw=True)

# plot_toe_trajectory_real_distance(raw_data, mouse_to_runs, phases=['APA2', 'Wash2'], savedir=step_save_dir, fs=7, n_interp=100)
# plot_toe_distance_to_transition(raw_data, mouse_to_runs, phases=['APA2', 'Wash2'], savedir=step_save_dir, fs=7, n_interp=100, stsw='st', bodypart='Toe', raw=True)
# plot_toe_distance_to_transition(raw_data, mouse_to_runs, phases=['APA2', 'Wash2'], savedir=step_save_dir, fs=7, n_interp=100, stsw='sw', bodypart='Toe', raw=True)
# plot_toe_distance_to_transition(raw_data, mouse_to_runs, phases=['APA2', 'Wash2'], savedir=step_save_dir, fs=7, n_interp=100, stsw='sw')
plot_toe_distance_to_transition(raw_data, mouse_to_runs, phases=['APA2', 'Wash2'], savedir=step_save_dir, fs=7, n_interp=100, stsw='st', bodypart='Tailbase')
plot_toe_distance_to_transition(raw_data, mouse_to_runs, phases=['APA2', 'Wash2'], savedir=step_save_dir, fs=7, n_interp=100, stsw='sw', bodypart='Tailbase')
# plot_toe_distance_to_transition(raw_data, mouse_to_runs, phases=['APA2', 'Wash2'], savedir=step_save_dir, fs=7, n_interp=100, stsw='sw', bodypart='Toe')
# plot_toe_distance_to_transition(raw_data, mouse_to_runs, phases=['APA2', 'Wash2'], savedir=step_save_dir, fs=7, n_interp=100, stsw='st', bodypart='Toe')
# plot_gait_features_corrs(data_LH['feature_data_notscaled'], stride=-1, phases=['APA2', 'Wash2'], savedir=angle_save_dir, fs=7)
# plot_nose_back_tail_averages(raw_data, mouse_to_runs, phases=['APA2', 'Wash2'], savedir=step_save_dir, fs=7, n_interp=100)
# plot_limb_positions_average(raw_data, mouse_to_runs, phases=['APA2', 'Wash2'], savedir=step_save_dir, fs=7, n_interp=100)
#
