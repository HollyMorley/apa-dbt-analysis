"""Plots of stride-level kinematic data and gait patterns."""
import os
...
def num_strides(raw_data):
    ...
    for condition, condition_data in raw_data.items():
        ...
        num_strides_belt2_df = pd.DataFrame(index=np.arange(0, 160), columns=mice)
        ...
        for mouse in mice:
            ...
            for ridx in mouse_data.index.get_level_values('Run').unique():
                ...
                # --- Belt 2 strides ---
                # Combine Transition and RunEnd stages
                belt_2_run = run_data.loc(axis=0)[['Transition', 'RunEnd']]
                stance_mask_b2 = belt_2_run.loc(axis=1)[
                                     transition_paw, 'SwSt_discrete'] == \
                                 locostuff['swst_vals_2025']['st']
                swing_mask_b2 = belt_2_run.loc(axis=1)[
                                    transition_paw, 'SwSt_discrete'] == \
                                locostuff['swst_vals_2025']['sw']

                stance_idxs_b2 = belt_2_run.index.get_level_values('FrameIdx')[
                    stance_mask_b2]
                swing_idxs_b2 = belt_2_run.index.get_level_values('FrameIdx')[
                    swing_mask_b2]

                strides_b2 = find_strides(stance_idxs_b2, swing_idxs_b2)
                num_strides_belt2_df.loc[ridx, mouse] = len(strides_b2)

        stride_counts_all[condition] = num_strides_df
        stride_counts_belt2_all[condition] = num_strides_belt2_df
    ...
    return stride_counts_all, stride_counts_belt2_all, sitting_log


def plot_num_strides_belt2(stride_counts_belt2_all):
    fig, axes = plt.subplots(1, 3, figsize=(15, 4), sharey=True)

    for ax, (condition, num_strides_df) in zip(axes,
                                               stride_counts_belt2_all.items()):
        ...
        ax.hist(all_counts, bins=bins,
                color=pu.get_color_speedpair(condition), edgecolor='white')

        ax.set_xlim(0, 10)
        ax.set_ylim(0,800)
        ax.set_xlabel('Number of strides on Belt 2')
        ax.set_title(condition)

    axes[0].set_ylabel('Number of trials')
    plt.tight_layout()

    save_path = os.path.join(savedir, f"stride_hist_belt2")
    ...
    for ax, (condition, stride_positions) in zip(axes, positions_all.items()):
        for label in stride_labels:
            ...
            for mouse_id, val in pairs:
                if np.isnan(val):
                    continue
                ax.scatter(val, label, color='grey',
                           marker='.',
...
def get_run_durations(raw_data, fps=247):
    durations_all = {}
    ...
    for condition, condition_data in raw_data.items():
        allowed_mice = \
            condition_specific_settings[CONDITION_CONFIG_MAP[condition]][
                'global_fs_mouse_ids']
        mice = [m for m in condition_data.keys() if m in allowed_mice]
        durations_df = pd.DataFrame(index=np.arange(0, 160), columns=mice)
        ...
        for mouse in mice:
            ...
            for ridx in mouse_data.index.get_level_values('Run').unique():
                ...
                belt_1_run = run_data.loc(axis=0)['RunStart']
                frame_idxs = belt_1_run.index.get_level_values('FrameIdx')
                run_start_idx = frame_idxs[0]
                duration_s = (transition_idx - run_start_idx) / fps
                durations_df.loc[ridx, mouse] = duration_s

        durations_all[condition] = durations_df

    return durations_all


def plot_run_durations(durations_all):
    fig, axes = plt.subplots(1, 3, figsize=(15, 4), sharey=True)

    for ax, (condition, durations_df) in zip(axes, durations_all.items()):
        all_durations = durations_df.values.flatten()
        all_durations = all_durations[~pd.isnull(all_durations)].astype(float)

        bins = np.arange(0, all_durations.max() + 0.1, 0.1)
        ax.hist(all_durations, bins=bins,
                color=pu.get_color_speedpair(condition), edgecolor='white')
    ...
    save_path = os.path.join(savedir, f"run_duration_hist")
    plt.savefig(f"{save_path}.png", dpi=400)
    plt.savefig(f"{save_path}.svg", dpi=400)
...
def get_run_duration_stats(durations_all):
    """
    Print descriptive statistics for run durations across all conditions,
    to support written claims about typical run duration.
    """
    for condition, durations_df in durations_all.items():
        all_durations = durations_df.values.flatten()
        all_durations = all_durations[~pd.isnull(all_durations)].astype(float)

        median = np.median(all_durations)
        p25, p75 = np.percentile(all_durations, [25, 75])
        p10, p90 = np.percentile(all_durations, [10, 90])
        mean = np.mean(all_durations)
        sem = np.std(all_durations) / np.sqrt(len(all_durations))

        print(f'{condition}: n={len(all_durations)} trials | '
              f'mean={mean:.2f}s (SEM={sem:.2f}) | '
              f'median={median:.2f}s | '
              f'IQR={p25:.2f}–{p75:.2f}s | '
              f'10th–90th={p10:.2f}–{p90:.2f}s')
...
stride_counts, stride_counts_belt2, sitting_log = num_strides(raw_data)
fig = plot_num_strides(stride_counts)
fig = plot_num_strides_belt2(stride_counts_belt2)

suspicious_runs = get_suspicious_runs(raw_data, stride_counts,
                                      above_threshold=6)

suspicious_runs_belt2_2 = get_suspicious_runs(raw_data, stride_counts_belt2, below_threshold=1)
...
# fig = plot_running_speeds(speeds_all, window=(-247, 0))
...
run_durations = get_run_durations(raw_data)
fig = plot_run_durations(run_durations)

get_run_duration_stats(run_durations)
